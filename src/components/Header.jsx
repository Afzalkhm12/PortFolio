...
function Header() {
  ...
  return (
    <header className="bg-white/80 backdrop-blur-sm sticky top-0 z-50 border-b border-[#45b0be]/20">
      <nav className="px-4 py-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        {/* Navigation wrapper */}
        <div className="flex items-center justify-between">
          {/* Logo */}
          <div 
            className="text-[#204a4e] text-3xl font-bold tracking-tight cursor-pointer"
          ...
          {/* Mobile menu button */}
          <button 
            className="md:hidden text-[#204a4e] hover:text-[#45b0be] transition-colors"
        ...
        {isMenuOpen && (
          <div className="md:hidden py-4 border-t border-[#204a4e]/10 mt-4">
...
function NavLinks({ mobile, scrollToSection }) {
  ...
  const baseClasses = "text-[#204a4e] hover:text-[#45b0be] transition-colors font-medium cursor-pointer"
...
function SocialIcons({ mobile }) {
  ...
  const socials = [
    { 
      name: 'GitHub',
      href: 'https://github.com/Afzalkhm12',
    ...
    { 
      name: 'LinkedIn',
      href: 'https://www.linkedin.com/in/afzal-khairahmansyach-1a0459248/',
  ...
  return (
    <div className={`flex ${mobile ? 'mt-4 justify-center' : ''} gap-4`}>
      {socials.map(social => (
        ...
        key={social.name}
        href={social.href} 
        className="text-[#204a4e] hover:text-[#45b0be] transition-colors"
